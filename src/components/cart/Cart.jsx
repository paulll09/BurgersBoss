...
const PAGE   = '#F5F0E8';
const CARD   = '#ffffff';
const CARD_SHADOW = '0 4px 24px rgba(0,0,0,0.07), 0 1px 4px rgba(0,0,0,0.04)';
...
export default function Cart() {
    ...
    const handleCheckout = async () => {
        ...
        text += `*Modalidad:* ${orderType === 'pickup' ? 'Retiro en el local' : 'Envio a domicilio'}\n`;
        if (orderType === 'delivery') {
            if (address) text += `*Direccion:* ${address}\n`;
            if (geoLocation) text += `*Ubicacion:* https://maps.google.com/?q=${geoLocation.lat},${geoLocation.lng}\n`;
        }
        text += `*Pago:* ${{ efectivo: 'Efectivo', transferencia: 'Transferencia' }[checkoutForm.paymentMethod]}\n`;
        if (notes) text += `*Aclaraciones:* ${notes}\n`;

        /* 3. Abrir WhatsApp — directo en móvil, wa.me en desktop */
    ...
    return (
        <div className="min-h-screen" style={{ background: PAGE }}>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {item.extras?.length > 0 && (
                                            <div className="mt-0.5 flex flex-col gap-0">
                                                {item.extras.map(e => (
                                                    <p key={e.id} className="font-body text-xs leading-snug" style={{ color: 'rgba(45,106,45,0.75)' }}>
                                                        + {e.name}
                                                    </p>
                                                ))}
                                            </div>
                                        )}
                                        <p className="font-body text-sm font-semibold mt-1" style={{ color: G }}>
                ...
                {orderType && (
                    ...
                    >
                        ...
                        <button
                            ...
                            disabled={submitting}
                            className="cursor-pointer w-full py-4 rounded-full font-body font-bold text-sm uppercase tracking-widest flex items-center justify-center gap-2.5 transition-all active:scale-[0.98] hover:opacity-90 disabled:opacity-60 disabled:cursor-not-allowed"
                            style={{ background: G_DARK, color: '#fff', boxShadow: '0 4px 20px rgba(26,74,26,0.35)' }}
                        >
            ...
            {orderType && (
                ...
                >
                    <div className="flex items-center gap-4 px-5 pt-4 max-w-2xl mx-auto">
                        <div className="shrink-0">
                            ...
                            {orderType === 'delivery' && (
                                <p className="font-body text-[10px]" style={{ color: DIM }}>+ envío</p>
                            )}
                        </div>
                        <button
                            onClick={handleCheckout}
                            disabled={submitting}
                            className="cursor-pointer flex-1 py-3.5 rounded-full font-body font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2 transition-all active:scale-[0.97] disabled:opacity-60 disabled:cursor-not-allowed"
                            style={{ background: G_DARK, color: '#ffffff', boxShadow: '0 4px 14px rgba(26,74,26,0.30)' }}
                        >
